...
Community scale fecal sludge treatment units (FSTUs) are an important technology in meeting the needs of the 2.5 billion people without adequate sanitation (**add citation**). Many of these communities also lack access to consistent electricity (**add citation**). To encourage the development of FSTUs that meet these needs, the International Standards Organization (ISO) is developing the ISO/PC318 standard for community scale resource oriented sanitation treatment systems(**add citation**. Key criteria for compliance with this emerging standard include: (1) the inactivation of pathogens present in fecal sludge and (2) energy independence or off-grid operation. EPA Rule, 40 CFR Part 503, Standards for the Use or Disposal of Sewage Sludge, is a widely adopted standard regulating the inactivation of pathogens in biosolids and provides guidance on how the first key criteria may be met across different treatment techniques including thermal treatment. High pH processes, composting, irradiation, and aerobic digestion. To meet the second key criteria, the treatment processes must be energy neutral or energy positive and operate on faecal sludge only without supplemental energy from other sources.
...
Collected data is stored in a standard relational cloud database that provides a flexible and dynamic data management platform **add citation**. Open source data science tools are used for data analysis, knowledge management and reporting toincrease reproducibility, collaboration and communication **add citation**. R Statistical Software and the RStudio Integrated Development Environment are used for data analysis **add citation**. The following open source R packages have been used from accessing data to producing the final manuscript: base, DBI, dbplyr, here, hms, lubridate, purrr, RMySQL, rstudioapi, snakecase, stringr, tidyverse **add citations**. 
...
The raw data underlying this study is available from Open Science Framework:

All computational code and additional data for reproduction of this manuscript is available from GitHub:
...
J. S. S. Lowndes et al., “Our path to better science in less time using open data science tools, ”Nature Ecology & Evolution, vol. 1, no. 6, pp. s41559–017–0160–017, May 2017.